...
export function WorkRequestIntakePage() {
  ...
  const updateRecommendedRateLabel = (category: string, rates = retainerRates) => {
    const mapping: Record<string, string> = {
      'Small Task': 'Manual Labor',
      'Event Support': 'Project Management',
      'Construction Project': 'Construction Supervision',
    };
    const type = mapping[category];
    if (!type) {
              ...
              <label key={cat} className="flex items-center gap-3 p-3 border border-neutral-200 hover:bg-neutral-50 cursor-pointer">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    updateRecommendedRateLabel(e.target.value);
                  }}
                  className="w-4 h-4 accent-primary-500"
                />
